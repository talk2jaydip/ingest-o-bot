...
graph TB
    ...
    subgraph "EMBEDDING STAGE"
        BATCH[Batch Formation<br/>---<br/>Group chunks<br/>Respect 8100 token limit]
        EMBED[Azure OpenAI Embeddings<br/>---<br/>text-embedding-3-large<br/>or ada-002<br/>Concurrent requests]
    end

    subgraph "VECTOR DATA"
        VECTORS[ChunkDocument with Embeddings<br/>---<br/>embedding: [0.123, -0.456, ...]<br/>dimensions: 1024 or 1536]
    end

    subgraph "INDEX STAGE"
        FORMAT[Search Document Format<br/>---<br/>Flatten metadata<br/>Add required fields<br/>Combine embeddings]
        BATCH2[Upload Batching<br/>---<br/>1000 documents per batch<br/>Merge-or-upload mode]
    end

    subgraph "OUTPUT"
        SEARCH[Azure AI Search Index<br/>---<br/>Indexed chunks with<br/>metadata, embeddings,<br/>and blob URLs]
    ...
    class TBL,IMG,CHUNK,BATCH,EMBED,FORMAT,BATCH2 process
    class SEARCH,ARTIFACTS output
```
...
**Batch Formation:**
```python
# Group chunks respecting token limit
batch_1 = [chunk1, chunk2, chunk3, ...]  # Total: 7800 tokens
batch_2 = [chunk17, chunk18, ...]         # Total: 6500 tokens
```
...
# Response
embeddings = [
    [0.123, -0.456, 0.789, ...],  # 1536 dims for ada-002
    [0.234, -0.567, 0.890, ...],  # 3072 dims for 3-large
]
...
### 7. Search Document Formatting

**Flatten ChunkDocument → Search Document:**
```python
{
    "id": "report_p1_c1",
    "chunk_id": "report_p1_c1",
...
await search_client.merge_or_upload_documents(batch_1)

# Batch 2: Chunks 1001-2000
await search_client.merge_or_upload_documents(batch_2)
```
