...
```bash
# === Parallel Processing ===
MAX_WORKERS=4                          # Number of documents to process concurrently
...
USE_INTEGRATED_VECTORIZATION=true      # Use server-side embeddings (fastest)
EMBEDDING_BATCH_SIZE=16                    # Embeddings per batch
UPLOAD_BATCH_SIZE=1000                 # Documents per upload batch

# === Input Configuration ===
INPUT_MODE=local                       # Input source: 'local' or 'blob'
LOCAL_INPUT_GLOB=documents/**/*.pdf          # Glob pattern for local files

# === Output Configuration ===
ARTIFACTS_MODE=blob                    # Store artifacts: 'blob' or 'local'
...
**Checklist:**
1. ✅ Enable integrated vectorization: `USE_INTEGRATED_VECTORIZATION=true`
