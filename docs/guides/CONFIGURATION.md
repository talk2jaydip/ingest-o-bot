...
**v2.0 Update**: The library now uses **generic naming** for provider-agnostic configuration:

- **Generic variables** (v2.0+): No prefix for provider-agnostic settings
  - `INPUT_MODE`, `CHUNKING_MAX_TOKENS`, `MAX_WORKERS`, etc.
- **Azure-specific variables**: Keep `AZURE_` prefix for Azure services
  - `AZURE_SEARCH_*`, `AZURE_OPENAI_*`, `AZURE_DOC_INT_*`, etc.
- **Backward compatibility**: Old `AZURE_`-prefixed generic variables work with deprecation warnings
- **Removal timeline**: Old names will be removed in v3.0
