...
# Set AZURE_INPUT_MODE=local
# Set AZURE_LOCAL_GLOB=./samples/*.pdf
...
AZURE_INPUT_MODE=blob
AZURE_STORAGE_CONTAINER=my-documents
AZURE_ARTIFACTS_DIR=./debug_artifacts  # Force local storage
```
...
- [ ] `AZURE_INPUT_MODE=local`
- [ ] `AZURE_LOCAL_GLOB` - Path pattern to files
