...
class EmbeddingsGenerator:
    ...
    def split_text_into_batches(self, texts: list[str]) -> list[EmbeddingBatch]:
        ...
        Uses the same batching logic as prepdocslib:
        - Respects token_limit (8100) per batch
        - Respects max_batch_size (16) items per batch
        """
        batch_info = self.SUPPORTED_BATCH_MODELS.get(self.model_name)
        if not batch_info:
            # Unsupported model - return each text as separate batch
            logger.warning(f"Model {self.model_name} not in supported batch models, using single-item batches")
            return [EmbeddingBatch(texts=[text], token_length=self._calculate_token_length(text)) for text in texts]
    ...
    async def generate_embeddings_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        logger.info(f"Split into {len(batches)} batches for parallel processing (max_concurrency={self.config.max_concurrency})")

        # Process batches in parallel (semaphore in _generate_batch_embeddings controls concurrency)
        tasks = [self._generate_batch_embeddings(batch) for batch in batches]
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                ...
                raise result  # Re-raise the first exception encountered
            all_embeddings.extend(result)

        logger.info(f"Generated {len(all_embeddings)} embeddings total using parallel batching")
...
def create_embeddings_generator(
    config: AzureOpenAIConfig,
    disable_batch: bool = False,
    max_retries: int = EmbeddingsGenerator.DEFAULT_MAX_RETRIES
) -> EmbeddingsGenerator:
