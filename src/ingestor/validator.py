...
class PipelineValidator:
        ...
        if self.config.input.mode == InputMode.LOCAL:
            ...
            if not glob_pattern:
                self.add_result(
                    False,
                    "Input Source (Local)",
                    "AZURE_LOCAL_GLOB is not set",
                    "Set AZURE_LOCAL_GLOB environment variable (e.g., './data/**/*.pdf')"
                )
                return
            ...
            if len(files) == 0:
                self.add_result(
                    ...
                    f"Ensure files exist at the specified path or adjust AZURE_LOCAL_GLOB"
                )
            else:
                self.add_result(
                    True,
    ...
    async def _validate_azure_openai(self):
        ...
        # Validate chat/GPT-4o configuration if needed for media description
        if self.config.media_describer_mode == MediaDescriberMode.GPT4O:
            if not self.config.azure_openai.chat_deployment:
                self.add_result(
                    False,
                    "Azure OpenAI (Chat/GPT-4o)",
                    "Chat deployment not configured (required for media description)",
                    "Set AZURE_OPENAI_CHAT_DEPLOYMENT environment variable or set AZURE_MEDIA_DESCRIBER=disabled"
                )
            else:
                self.add_result(
                    True,
                    "Azure OpenAI (Chat/GPT-4o)",
                    f"Chat deployment configured: {self.config.azure_openai.chat_deployment}"
                )
