...
class BlobArtifactStorage(ArtifactStorage):
    ...
    async def _get_blob_service_client(self) -> BlobServiceClient:
        ...
        if self._blob_service_client is None:
            # Priority: connection_string > credential (account key) > DefaultAzureCredential
    ...
    async def ensure_containers_exist(self):
        ...
        # Check all containers in parallel for efficiency
        async def check_and_create_container(container_name: str):
        ...
        import asyncio
        await asyncio.gather(*[check_and_create_container(c) for c in containers_to_create])
