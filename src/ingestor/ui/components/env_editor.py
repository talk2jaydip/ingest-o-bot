...
ENV_GROUPS = {
    ...
    "Office Extraction": [
        ...
        "OFFICE_OFFLINE_FALLBACK",
        "OFFICE_EQUATION_EXTRACTION",
...
def create_env_editor() -> tuple:
    ...
    def validate_config(**kwargs):
        ...
        input_mode = kwargs.get("INPUT_MODE") or os.getenv("INPUT_MODE", "local")
        if input_mode == "blob":
            ...
            local_glob = kwargs.get("LOCAL_INPUT_GLOB") or os.getenv("LOCAL_INPUT_GLOB")
            if not local_glob:
                issues.append("❌ Missing LOCAL_INPUT_GLOB (required for local input)")
